...
import { useTransactionExecution } from '../internal/useTransactionExecution'
...
export const useTokenAllowance = (
  ...
  const { positiveVersion } = useGeneral()
...
export const useTokenApprove = (
  ...
  const { positiveReload, appTheme } = useGeneral()
  ...
  const approve = useCallback(
    async (tokenAddr: string, tokenAbi: any, spender: string, amount?: BigNumber) => {
      ...
      try {
        ...
        makeTxToast(FunctionName.APPROVE, TransactionCondition.PENDING, appTheme, txHash)
        await tx.wait(activeNetwork.rpc.blockConfirms).then((receipt: TransactionReceipt) => {
          const status = receipt.status ? TransactionCondition.SUCCESS : TransactionCondition.FAILURE
          makeTxToast(FunctionName.APPROVE, status, appTheme, txHash)
          positiveReload()
        })
