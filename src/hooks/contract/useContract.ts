...
import { ContractSources, Network } from '../../constants/types'
...
export function useContractArray(_activeNetwork: Network): ContractSources[] {
  return useMemo(() => {
    ...
    Object.keys(config.generalContracts).forEach((key) => {
      if (!excludedContractAddrs.includes(config.generalContracts[key].addr)) {
        contractSources.push({
          addr: config.generalContracts[key].addr.toLowerCase(),
          abi: config.generalContracts[key].abi,
        })
      }
    })
